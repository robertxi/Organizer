...
whatDoApp.controller('updateDropDownCtrl', function ($scope) {
...
whatDoApp.controller('ModalInstanceCtrl', function ($scope, $http, $modalInstance, $rootScope) {
    ...
    $scope.$watch(function ($scope) {
            return $scope.newuser.confpassword
        },
        function (newValue) {
    ...
    $scope.register = function () {
        ...
        getuname.success(function (data) {
            if (data == true) {
                ...
                var res = $http.post('http://localhost:8080/registerUser', newdata);
                res.success(function () {
                    $modalInstance.close();
                });
                res.error(function () {
                    alert("failure message: " + JSON.stringify({data: newdata}));
                    $modalInstance.close();
                });
            }
        });
...
whatDoApp.controller('toDoListCtrl', function ($scope, $http, $uibModal, $rootScope, $cookieStore, $location) {
    ...
    $scope.submitComment = function (item) {
        ...
        var data = {
            'taskitem_id': item.id,
            'content': $scope.comment.newComment,
            'date_created': date,
            'token': $rootScope.authToken
        };
        var res = $http.post('http://localhost:8080/addNewComment', data);
    ...
    $scope.removeTaskItem = function (index, taskItem) {
        var data = {
